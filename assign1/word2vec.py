...
def naive_softmax_loss(
    ...
    vc = center_vectors[center_word_index].T            # batch size x word_dim
   
    # naive softmax function
    Uv = -1 * F.log_softmax(torch.matmul(outside_vectors[1:, :], vc).T, dim=1)

    # Tensor for zero padding at PAD position
...
def neg_sampling_loss(
    ...
    # calculate uv without PAD layer
    Uv = torch.matmul(outside_vectors[1:,:], center_vectors[center_word_index].T).T
 
    # Tensor for zero padding at PAD position
    result = Uv.new_zeros((2, batch_size, n_tokens), dtype=Uv.dtype, requires_grad=True)
      
    # calculate sigmoid and log (as following the loss function of negative sampling)
    result[0, :, 1:] = -1 * F.logsigmoid(Uv)        # for outside words
    result[1, :, 1:] = -1 * F.logsigmoid(-1 * Uv)   # for negative samples
    
    # if outside word's index is 0(PAD), negative samples for PAD don't be calculated
    negative_samples[outside_word_indices.unsqueeze(-1).expand(-1, -1, K) == 0] = 0
    
    # losses : [batch size]
    losses = torch.gather(result[0], 1, outside_word_indices).sum(dim=1) + \
            torch.gather(result[1], 1, negative_samples.reshape(batch_size, -1)).sum(dim=1)
