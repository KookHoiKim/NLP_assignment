...
def build_vocab(
    ...
    min_freq -- The minimum frequency of a word to be a seperate token.
    ...
    # count every word in sentences
    counterWord = Counter()
    for sen in sentences:
        counterWord.update(sen)
    
    # select words that counting is >= than min_freq, else plus UNK values
    newdict = {}
    for i in counterWord.keys():
        if counterWord[i] >= min_freq:
            newdict[i] = counterWord[i]
        else:
            word_freq[UNK_idx] += 1
    
    idx2word = idx2word + list(newdict)
    word2idx = {idx2word[word] : word for word, i in enumerate(idx2word)}
    word_freq = word_freq + list(newdict.values())
...
def skipgram(
    ...
    start = center_word_loc - window_size if center_word_loc >= window_size\
            else 0
    end = center_word_loc + window_size if center_word_loc < lenSen - window_size\
            else lenSen - 1
